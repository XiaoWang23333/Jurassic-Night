...
const Player = {
  ...
  atk:14, def:4, atb:0, atbMax:100, atbSpeed:24,
  ...
  skills:[
    ...
    { id:'focus',   name:'集中精神', cost:5,  desc:'立刻填满ATB槽',         buff:'atbFull' },
  ],
  items:[
...
const EnemyTpl = {
  velo:   { name:'迅猛龙', hp:55,  atk:11, def:1, atbSpeed:30, color:0x6faa55, scale:1.0, type:'raptor', speed:2.4, atkRange:1.7, atkPrep:0.7, atkAoe:0   },
  pter:   { name:'翼龙',   hp:42,  atk:9,  def:0, atbSpeed:38, color:0x9b6bbd, scale:0.9, type:'ptero',  speed:3.0, atkRange:1.6, atkPrep:0.6, atkAoe:0   },
  tricer: { name:'三角龙', hp:110, atk:16, def:5, atbSpeed:18, color:0x8a6a3a, scale:1.3, type:'tricer', speed:1.8, atkRange:2.1, atkPrep:1.0, atkAoe:0   },
  rex:    { name:'霸王龙', hp:220, atk:22, def:6, atbSpeed:20, color:0x5a3a2a, scale:1.7, type:'rex',    speed:2.0, atkRange:2.6, atkPrep:1.1, atkAoe:2.5 },
};
...
function buildAreaCamp(){
  G.scene.add(makeGroundPatch(40, 40, 0x2c3a30));
  G.scene.add(new THREE.Mesh(new THREE.SphereGeometry(45,16,12),
    new THREE.MeshBasicMaterial({color:0x162230, side:THREE.BackSide, fog:false})));
  G.scene.add(makeCampfire(new THREE.Vector3(3.2,0,4)));
  const t1 = makeTent(); t1.position.set(5,0,5); G.scene.add(t1);
  const t2 = makeTent(); t2.position.set(-5,0,5); t2.rotation.y=0.4; G.scene.add(t2);
  for(let i=0;i<3;i++){ const c = makeCrate(); c.position.set(-4+i*0.9, 0, 6.5); G.scene.add(c); }
  for(let i=0;i<26;i++){
    const a = i / 26 * Math.PI*2;
    const r = 11 + Math.random()*1.5;
    const x = Math.cos(a)*r, z = Math.sin(a)*r;
    if(Math.abs(x)<2 && z<-8) continue;
  ...
  for(let i=0;i<6;i++){
    const r = makeRock();
    r.position.set(-6+Math.random()*12, 0, -2+Math.random()*8);
    r.scale.setScalar(0.4+Math.random()*0.8);
    G.scene.add(r);
  }
  const path = new THREE.Mesh(new THREE.PlaneGeometry(2.4, 3.5),
    new THREE.MeshBasicMaterial({color:0xb09040, transparent:true, opacity:0.25}));
  path.rotation.x = -Math.PI/2; path.position.set(0, 0.02, -8.3);
  G.scene.add(path);
}
function buildAreaJungle(){
  G.scene.add(makeGroundPatch(28, 50, 0x223028));
  G.scene.add(new THREE.Mesh(new THREE.SphereGeometry(45,16,12),
    new THREE.MeshBasicMaterial({color:0x101a22, side:THREE.BackSide, fog:false})));
  for(let z=8; z>-22; z-=1.4){
    for(const side of [-1,1]){
      const t = makeTree();
      const x = side * (8 + Math.random()*2.5);
      t.position.set(x + (Math.random()-0.5)*1.5, 0, z + (Math.random()-0.5)*0.8);
  ...
  for(let i=0;i<10;i++){
    const r = makeRock();
    r.position.set((Math.random()-0.5)*9, 0, 4 - i*2.5 + Math.random()*0.8);
    r.scale.setScalar(0.5+Math.random()*0.7);
    G.scene.add(r);
  }
  const fallen = new THREE.Mesh(new THREE.CylinderGeometry(0.45,0.5,5,6),
    new THREE.MeshStandardMaterial({color:0x2a1f12, flatShading:true}));
  fallen.rotation.z = Math.PI/2; fallen.position.set(0,0.4,-6); fallen.castShadow=true;
  G.scene.add(fallen);
  for(let z=6; z>-20; z-=4){
    const path = new THREE.Mesh(new THREE.PlaneGeometry(1.6, 0.5),
      new THREE.MeshBasicMaterial({color:0x6a5028, transparent:true, opacity:0.18}));
    path.rotation.x = -Math.PI/2; path.position.set(0, 0.02, z);
    G.scene.add(path);
  }
  const w1 = makeCrate(); w1.position.set(-1.2,0,-12); w1.rotation.y=0.4; G.scene.add(w1);
  const w2 = makeCrate(); w2.position.set(1,0,-13); w2.rotation.y=-0.5; G.scene.add(w2);
}
function buildAreaNest(){
  G.scene.add(makeGroundPatch(40, 40, 0x2a221a));
  G.scene.add(new THREE.Mesh(new THREE.SphereGeometry(45,16,12),
    new THREE.MeshBasicMaterial({color:0x1a0d10, side:THREE.BackSide, fog:false})));
  const boneMat = new THREE.MeshStandardMaterial({color:0xc8c0a8, flatShading:true});
  for(let i=0;i<14;i++){
    const b = new THREE.Mesh(new THREE.CylinderGeometry(0.08,0.08,1.0+Math.random()*0.6,5), boneMat);
    const a = Math.random()*Math.PI*2, r=2+Math.random()*5;
    b.position.set(Math.cos(a)*r, 0.1, Math.sin(a)*r);
    b.rotation.z = Math.random()*Math.PI; b.rotation.y = Math.random()*Math.PI;
    G.scene.add(b);
  }
  for(let i=0;i<6;i++){
    const a = i/6 * Math.PI*2;
    const r = 11 + Math.random()*1;
    const rock = new THREE.Mesh(
      new THREE.CylinderGeometry(0.7+Math.random()*0.5, 1.2, 2.5+Math.random()*1.5, 5),
      new THREE.MeshStandardMaterial({color:0x4a3528, flatShading:true}));
    rock.position.set(Math.cos(a)*r, 1.2, Math.sin(a)*r); rock.castShadow=true;
    G.scene.add(rock);
  }
  for(let i=0;i<14;i++){
    const a = i/14 * Math.PI*2;
    const r = 13 + Math.random()*1.5;
    const t = makeTree(); t.position.set(Math.cos(a)*r, 0, Math.sin(a)*r);
    t.scale.setScalar(0.7+Math.random()*0.4);
    G.scene.add(t);
  }
  const red = new THREE.PointLight(0xaa3322, 1.2, 18, 2);
  red.position.set(0, 3, -2); G.scene.add(red);
}

function setupAreas(){
  Areas.length = 0;
  Areas.push({
    name:'营地', bounds:{minX:-9, maxX:9, minZ:-9, maxZ:9},
    spawn: new THREE.Vector3(2, 0, 6),
    cams:[{ pos:new THREE.Vector3(0, 9, 12), look:new THREE.Vector3(0, 0.5, 0), fov:55 }],
    exit:{ pos:new THREE.Vector3(0, 0, -8), radius:1.4, label:'前往丛林' },
    battle:{
      trigger:{ pos:new THREE.Vector3(0,0,-2), radius:2.0 },
      arena:{ center:new THREE.Vector3(0,0,-1), radius:6.5 },
      enemies:['velo','velo'],
    },
    build: buildAreaCamp,
  });
  Areas.push({
    name:'迷雾丛林', bounds:{minX:-7, maxX:7, minZ:-22, maxZ:9},
    spawn: new THREE.Vector3(0, 0, 7),
    cams:[
      { pos:new THREE.Vector3(0, 8.5, 12), look:new THREE.Vector3(0, 0.5, 2), fov:55, range:[-3, 999] },
      { pos:new THREE.Vector3(0, 8.5, -2), look:new THREE.Vector3(0, 0.5, -12),fov:55, range:[-999, -3] },
    ],
    exit:{ pos:new THREE.Vector3(0, 0, -20), radius:1.4, label:'进入巢穴' },
    battle:{
      trigger:{ pos:new THREE.Vector3(0,0,-12), radius:2.2 },
      arena:{ center:new THREE.Vector3(0,0,-12), radius:6.5 },
      enemies:['velo','pter','tricer'],
    },
    build: buildAreaJungle,
  });
  Areas.push({
    name:'霸王龙巢穴', bounds:{minX:-9, maxX:9, minZ:-9, maxZ:9},
    spawn: new THREE.Vector3(0, 0, 7.5),
    cams:[{ pos:new THREE.Vector3(0, 10, 13), look:new THREE.Vector3(0, 1.0, -1), fov:50 }],
    battle:{
      trigger:{ pos:new THREE.Vector3(0,0,3), radius:2.5 },
      arena:{ center:new THREE.Vector3(0,0,-1), radius:7.5 },
...
function initThree(){
  ...
  G.scene.fog = new THREE.Fog(0x1a2a38, 22, 55);
...
function addBaseLights(){
  const moon = new THREE.DirectionalLight(0xcfe0f0, 1.4);
  moon.position.set(-6,14,6); moon.castShadow=true;
  moon.shadow.mapSize.set(1024,1024);
  Object.assign(moon.shadow.camera, { left:-18,right:18,top:18,bottom:-18 });
  G.scene.add(moon);
  // 半球光让上下都有色彩，不至于死黑
  G.scene.add(new THREE.HemisphereLight(0x9bb8d0, 0x3a3528, 0.85));
  G.scene.add(new THREE.AmbientLight(0x6b7e92, 0.6));
}
...
function loadArea(idx){
  ...
  G.scene.add(m);
  G.player = { mesh:m, anim:0, atkTime:0, hurtTime:0, walkPhase:0 };
...
function resumeGame(){
  G.paused = false;
  if(G.state==='battle'){ modeBadge.textContent = '⚔ 战斗'; modeBadge.classList.add('danger'); }
  else if(G.state==='explore'){ modeBadge.textContent = '探索中'; modeBadge.classList.remove('danger'); }
}
function updateExploreHints(){
  ...
  let nearExit = null;
  if(area.exit && p.distanceTo(area.exit.pos) < area.exit.radius){
    nearExit = area.exit;
  }
  if(nearExit){
    actBtnEl.textContent = nearExit.label;
    actBtnEl.classList.add('show');
    G.actCallback = ()=>{ goNextArea(); };
  } else {
    actBtnEl.classList.remove('show');
    G.actCallback = null;
  }
...
function goNextArea(){
  const next = G.areaIdx+1;
  if(next >= Areas.length) return;
  setMode('cinema');
  fadeEl.classList.add('show');
  setTimeout(()=>{ loadArea(next); fadeEl.classList.remove('show'); }, 500);
}
...
function enterBattle(){
  ...
  cfg.enemies.forEach((type,i)=>{
    ...
    G.enemies.push({
      ...
      atb: 20 + Math.random()*30, atbMax:100, atbSpeed: tpl.atbSpeed,
...
function tryPlayerAction(kind){
  ...
  if(kind==='attack'){ requestTarget('attack'); }
    ...
    // 关闭菜单并恢复游戏；ATB 仍为满，下次按攻击/技能可继续选目标
    cmdMenu.classList.remove('show');
    hideSubMenu();
    G.pendingAction = null;
    atbHint.classList.add('hide');
    // 让玩家有自由跑动空间：把 ATB 减一点，避免立刻又满进暂停
    Player.atb = Player.atbMax * 0.6;
    updateHUD();
    resumeGame();
  }
...
function executeSkill(skill, targetIdx){
  ...
  resumeGame();
  if(skill.id==='focus'){
    Player.atb = Player.atbMax;
    showDmg(G.player.mesh.position.clone().add(new THREE.Vector3(0,2,0)),'集中!','heal');
    log('集中精神，ATB 充满'); updateHUD(); return;
  }
  if(skill.aoe){
    const t = G.enemies[targetIdx];
    if(!t) return;
      ...
      ()=>{
        ...
        G.enemies.filter(e=>e.alive).forEach(e=>{
          if(e.mesh.position.distanceTo(center) <= skill.aoe){
            const dmg = Math.floor((Player.atk - e.def + 4) * skill.dmg + Math.random()*5);
            e.hp -= dmg; e.hurtTime=0.001;
            showDmg(e.mesh.position, dmg, 'crit enemy');
    ...
    setTimeout(()=>{
      ...
      fireBullet(
        ...
        ()=>{
          ...
          if(skill.crit) dmg = Math.floor(dmg*1.5);
          t.hp -= dmg; t.hurtTime=0.001;
          showDmg(t.mesh.position, dmg, skill.crit?'crit enemy':'enemy');
          if(t.hp<=0) killEnemy(t);
          checkBattleEnd(); updateHUD();
        }
      );
...
function updateEnemyAI(e, dt){
  ...
  // 恐龙的"头"朝 -Z 方向，需要让 -Z 指向玩家：yaw = atan2(-x, -z) = atan2(x,z) + π
  ...
  if(e.aiState === 'chase'){
    ...
    if(e.atb >= e.atbMax && dist <= e.tpl.atkRange*1.2){
      e.aiState = 'prepare';
...
function refreshEnemyTags(){
  ...
  G.enemies.forEach((e,idx)=>{
    ...
    div.querySelector('.nm').textContent = e.tpl.name;
...
function showSubMenu(kind){
  ...
  if(kind==='skill'){
    Player.skills.forEach(s=>{
      ...
      div.onclick = ()=>{
        hideSubMenu();
        if(s.id==='focus'){ executeSkill(s, -1); }
        else { requestTarget('skill', s); }
      };
...
function startGame(){
  ...
  Player.atb = 0; Player.alive = true; Player.buffs.haste = 0;
  Player.items.forEach(it=>{
